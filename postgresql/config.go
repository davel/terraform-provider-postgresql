	...
	if !found {
		...
		if c.config.Scheme == "postgres" {
			if c.config.HostAddr != "" {
				db, err = sql.OpenDB(pq.DialOpen(&driverWrapper{c.config.HostAddr}, dsn))
		...
		if err != nil {
			...
			return nil, fmt.Errorf("Error connecting to PostgreSQL server %s (scheme: %s): %s", dsn, c.config.Scheme, errString)
		}
